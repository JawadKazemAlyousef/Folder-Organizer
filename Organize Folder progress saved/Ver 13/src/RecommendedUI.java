	...
	public void helpSetting(OrganizeFolderMainUI mainObj) {
		...
		if(mainObj.getLanguageMode() == 0) {
			    ...
			    Text partTwo = new Text("to select the path (the folder location)\n\n"+
			    		 ...
			    		 "3. Choose the language that you want folder's names based on\n\n"+
						 "4. Make sure to save the settings by pressing ");
			    ...
			    Text partSix = new Text("to organize the folder's files based on my recommended choice\n\n"+
			    						"Note: If there are extesions format that do not in the recommended mode list, they will be automatically organized based on their extensions format");
			    ...
			    flow.getChildren().addAll(rMode, partOne, bChoose, partTwo, mFile, partThree, mSave, partFour, mSaveAs, partFive, bOrganize, partSix);
		    ...
		    Text partTwo = new Text("لتختار المسار (موقع المجلد)\n\n"+
					 ...
					 "4. تأكد من أنك حفظت الإعدادات وهذا عن طريق نقر ");
		    ...
		    Text partSix = new Text("لتنظم ملفات المجلد المختار حسب ما استحسنه\n\n"+
		    						"ملاحظة: إذا هناك صيغ غير موجودة في القائمة الموصى بها، سترتب تلقائيا حسب صيغهم");
		...
		if(mainObj.getInitialSavedSetting() != 1) {
			...
			String[][] extenstionsForEachFolderR = { 
													...
													{"rar", "zip", "7z", "s7z", "ace", "afa", "alz",  "arc", "ark", "cdx", "arj", "b1", "b6z", "ba", "bh",
													 ...
													 "lz", "lz4", "lzma", "lzo", "rz", "sfark", "sz", "?Q?", "q", "?q?", "?Z?", "xz", "z", "Z", "zst", "??_"},
			...
			for (int i=0; i< folderNamesR.length; ++i) {
				...
				for(int j=0; j< folderContents.length; ++j) {
					...
					for(int k=0; k< extenstionsForEachFolderR[i].length; ++k) {
						if(organizedChecker.isFile() && folderContents[j].substring(folderContents[j].lastIndexOf(".")+1).equals(extenstionsForEachFolderR[i][k])){
